...
func startLogin(c string,kind string) error{sendProgress("Preparando o login do ChatGPT...");if err:=ensureMcp(c);err!=nil{return err};sendProgress("Abrindo o login do ChatGPT no navegador...");var cmd *exec.Cmd;if kind=="chatgpt"{cmd=exec.Command(c,"login")}else{cmd=exec.Command(c,"mcp","login",kind)};if err:=cmd.Start();err!=nil{return err};return nil}
