...
func startLogin(c string,kind string) error{if err:=ensureMcp(c);err!=nil{return err};if kind=="chatgpt"{cmd:=exec.Command("cmd.exe","/c","start","","LovInfinity - Login ChatGPT",c,"login");if err:=cmd.Run();err!=nil{return err};return nil};cmd:=exec.Command("cmd.exe","/c","start","","LovInfinity - Login MCP",c,"mcp","login",kind);if err:=cmd.Run();err!=nil{return err};return nil}
...
USER REQUEST:
`+prompt;cmd:=exec.Command(c,"exec","--sandbox","danger-full-access","--ephemeral",finalPrompt);var out bytes.Buffer;cmd.Stdout=&out;cmd.Stderr=&out;err:=cmd.Run();s:=strings.TrimSpace(out.String());if err!=nil{return s,fmt.Errorf("codex_error: %v%s",err,func()string{if s!=""{return ": "+s};return ""}())};return s,nil}
