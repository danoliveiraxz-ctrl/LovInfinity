...
func readNative()([]byte,error){
 ...
 n:=binary.LittleEndian.Uint32(h[:]);if n>4<<20{return nil,fmt.Errorf("message too large")}
...
func codexPath() string {
 if p,e:=exec.LookPath("codex");e==nil{return p}
 if p,e:=exec.LookPath("codex.exe");e==nil{return p}
 candidates:=[]string{
...
func installCodex() error {
 ...
 if e:=cmd.Run();e!=nil{return fmt.Errorf("codex_install_failed: %v %s",e,strings.TrimSpace(out.String()))}
 if codexPath()==""{return fmt.Errorf("codex_installed_but_not_found")}
 return nil
}
func runCmd(bin string,args ...string)(string,error){
 ...
 if e!=nil{return s,fmt.Errorf("%v%s",e,func()string{if s!=""{return ": "+s};return ""}())}
 return s,nil
}
func ensureMcp(c string) error {
 list,err:=runCmd(c,"mcp","list");if err!=nil{return err}
 if !strings.Contains(list,"github"){if _,err=runCmd(c,"mcp","add","github","--url","https://api.githubcopilot.com/mcp/");err!=nil{return err}}
 if !strings.Contains(list,"lovable"){if _,err=runCmd(c,"mcp","add","lovable","--url","https://mcp.lovable.dev/");err!=nil{return err}}
 return nil
}
func startLogin(c string,kind string) error {
 if err:=ensureMcp(c);err!=nil{return err}
 var cmd *exec.Cmd
 if kind=="chatgpt"{cmd=exec.Command(c,"login")}else{cmd=exec.Command(c,"mcp","login",kind)}
 if err:=cmd.Start();err!=nil{return err}
 return nil
}
func runCodex(prompt string)(string,error){
 c:=codexPath();if c==""{if err:=installCodex();err!=nil{return "",err};c=codexPath()}
 if err:=ensureMcp(c);err!=nil{return "",err}
...
USER REQUEST:
`+prompt
 cmd:=exec.Command(c,"exec","--sandbox","danger-full-access","--ephemeral",finalPrompt)
 var out bytes.Buffer;cmd.Stdout=&out;cmd.Stderr=&out
 err:=cmd.Run()
 s:=strings.TrimSpace(out.String())
 if err!=nil{return s,fmt.Errorf("codex_error: %v%s",err,func()string{if s!=""{return ": "+s};return ""}())}
 return s,nil
}
func main(){
 ...
 if m.Action=="ensure_server"||m.Action==""{
  startServer();time.Sleep(100*time.Millisecond);_=writeNative(Resp{Ok:true,Port:port})
  for{b,e=readNative();if e!=nil{return};var x Msg;if json.Unmarshal(b,&x)!=nil{_=writeNative(Resp{Ok:false,Error:"invalid_message"});continue}
   switch x.Action{
   case "codex_status":
    c:=codexPath();if c==""{_=writeNative(Resp{Ok:false,Error:"codex_not_installed"});continue}
    out,err:=runCmd(c,"--version");if err!=nil{_=writeNative(Resp{Ok:false,Error:err.Error()})}else{_=writeNative(Resp{Ok:true,Output:out})}
   case "codex_install":
    err:=installCodex();if err!=nil{_=writeNative(Resp{Ok:false,Error:err.Error()})}else{_=writeNative(Resp{Ok:true,Installed:true})}
   case "codex_login":
    c:=codexPath();if c==""{if err:=installCodex();err!=nil{_=writeNative(Resp{Ok:false,Error:err.Error()});continue};c=codexPath()}
    if err:=startLogin(c,"chatgpt");err!=nil{_=writeNative(Resp{Ok:false,Error:err.Error()})}else{_=writeNative(Resp{Ok:true,Output:"login_started"})}
   case "codex_mcp_login":
    c:=codexPath();if c==""{_=writeNative(Resp{Ok:false,Error:"codex_not_installed"});continue}
    kind:=x.Prompt;if kind!="github"&&kind!="lovable"{_=writeNative(Resp{Ok:false,Error:"invalid_mcp_provider"});continue}
    if err:=startLogin(c,kind);err!=nil{_=writeNative(Resp{Ok:false,Error:err.Error()})}else{_=writeNative(Resp{Ok:true,Output:kind+"_login_started"})}
   case "run_codex":
    if strings.TrimSpace(x.Prompt)==""{_=writeNative(Resp{Ok:false,Error:"missing_prompt"});continue}
    out,err:=runCodex(x.Prompt);if err!=nil{_=writeNative(Resp{Ok:false,Error:err.Error(),Output:out})}else{_=writeNative(Resp{Ok:true,Output:out})}
   default:_=writeNative(Resp{Ok:false,Error:"unknown_action"})
   }
  }
 }
