...
async function loadConnections(){if(!activeSession)return;try{const d=await api({action:"list",session_token:activeSession});const c=d.connections||[];for(const p of["github","lovable","openai"]){const x=c.find(v=>v.provider===p);const state=$(p+"State"),btn=$(p+"Connect");state.textContent=x?(x.external_account_name||"Conectado"):"Não conectado";if(p==="lovable")btn.textContent=x?"Reconectar com Lovable":"Conectar com Lovable";else btn.textContent=x?"Reconectar":"Conectar";}}catch(e){}}
