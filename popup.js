...
async function loadConnections(){if(!activeSession)return;try{const d=await api({action:"list",session_token:activeSession});const c=d.connections||[];for(const p of["github","lovable","openai"]){const x=c.find(v=>v.provider===p);const state=$(p+"State"),btn=$(p+"Connect");state.textContent=x?(x.external_account_name||"Conectado"):"Não conectado";btn.textContent=x?"Reconectar":"Conectar";}}catch(e){}}
...
$("githubConnect").onclick=async()=>{try{const d=await api({action:"github_start",session_token:activeSession});if(!d.url)throw Error("GitHub OAuth não configurado.");chrome.tabs?chrome.tabs.create({url:d.url}):window.open(d.url,"_blank");setTimeout(loadConnections,4000);setTimeout(loadConnections,9000)}catch(e){$("githubState").textContent=e.message}};
for(const p of["lovable","openai"]){$(p+"Connect").onclick=()=>{pendingProvider=p;$("keyTitle").textContent="Conectar "+(p==="lovable"?"Lovable":"OpenAI");$("apiKey").value="";$("keyError").textContent="";$("keyBox").classList.remove("hidden");};}
