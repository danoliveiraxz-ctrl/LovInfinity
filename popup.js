...
async function loadConnections(){if(!activeSession)return;try{const d=await api({action:"list",session_token:activeSession});const c=d.connections||[];for(const p of["github","lovable"]){const x=c.find(v=>v.provider===p);const state=$(p+"State"),btn=$(p+"Connect");state.textContent=x?(x.external_account_name||"Conectado"):"Não conectado";if(p==="lovable"){btn.textContent=x?"Reconectar com Lovable":"Conectar com Lovable";if(x)await loadLovableProjects();else $("lovableProjectBox").classList.add("hidden");}else btn.textContent=x?"Reconectar":"Conectar";}}catch(e){}}
...
async function ensureLovableHost(){
  return new Promise((resolve,reject)=>{
    chrome.runtime.sendMessage({action:"ensure_lovable_host"},r=>{
      if(chrome.runtime.lastError)return reject(Error("Componente local LovInfinity não instalado. Execute install.bat da pasta do LovInfinity."));
      if(!r?.ok)return reject(Error(r?.error||"Não foi possível iniciar o componente local."));
      resolve(r);
    });
  });
}
$("lovableConnect").onclick=async()=>{
  const btn=$("lovableConnect");
  try{
    btn.disabled=true;
    $("lovableState").textContent="Iniciando conexão segura...";
    await ensureLovableHost();
    const d=await api({action:"lovable_start",session_token:activeSession});
    if(!d.url)throw Error("OAuth da Lovable não configurado.");
    chrome.tabs.create({url:d.url});
    let tries=0;
    const poll=setInterval(async()=>{
      tries++;
      await loadConnections();
      if($("lovableState").textContent!=="Não conectado"||tries>=20){
        clearInterval(poll);
        btn.disabled=false;
      }
    },2000);
  }catch(e){
    $("lovableState").textContent=e.message;
    btn.disabled=false;
  }
};
