...
async function ensureLovableHost(){return new Promise((resolve,reject)=>{chrome.runtime.sendNativeMessage("com.lovinfinity.oauth",{action:"ensure_server"},r=>{if(chrome.runtime.lastError)return reject(Error("Componente local LovInfinity não instalado. Execute install.bat da pasta do LovInfinity."));if(!r?.ok)return reject(Error(r?.error||"Não foi possível iniciar o componente local."));resolve(r)})})}\n$("lovableConnect").onclick=async()=>{const btn=$("lovableConnect");try{btn.disabled=true;$("lovableState").textContent="Iniciando conexão segura...";await ensureLovableHost();const d=await api({action:"lovable_start",session_token:activeSession});if(!d.url)throw Error("OAuth da Lovable não configurado.");chrome.tabs.create({url:d.url});let tries=0;const poll=setInterval(async()=>{tries++;await loadConnections();if($("lovableState").textContent!=="Não conectado"||tries>=20){clearInterval(poll);btn.disabled=false}},2000)}catch(e){$("lovableState").textContent=e.message;btn.disabled=false}};
